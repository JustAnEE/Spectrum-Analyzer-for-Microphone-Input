...
void ListMenu::scrollOptions(double xpos, double ypos, int direction)
{
    ...
    if (optionStrings.size() < optionsShown)
    {
        return;
    }
    ...
    if (  (windowStart == 0 && direction == -1) || (windowStart == upperLimit && direction == 1)  )
    {
        return;
    }
...
void ListMenu::fillOptionText()
{
    ...
    {
        float y = optionHeight - ((i + 1) / 7.0) + bottom;
        textList.push_back(new TextLabel(glm::vec3(location.x, y, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), width * 0.9f, optionStrings[windowStart + i]));
    }
