...
{ 
    ...
    switch (flag)
    {
        ...
        case 3:
            ...
            break;
        default:
            std::cout << "Invalid function flag in SpectrumModel::addPlot." << "\n";
            exit(-69);
            break;
    }
...
void Plot::setWindowFlag(int flag)
{
    WINDOWFLAG = flag;
}

void Plot::setFilterFlag(int flag)
{
    FILTERFLAG = flag;
}

void Plot::setDetrendFlag(bool flag)
{
    DETREND = flag;
}

void Plot::setNormalizeFlag(bool flag)
{
    NORMALIZE = flag;
};

int  Plot::getWindowFlag()
{
    return WINDOWFLAG;
}

int  Plot::getMethodFlag()
{
    return METHODFLAG;
}

int  Plot::getFilterFlag()
{
    return FILTERFLAG;
}

bool Plot::getDetrendFlag()
{
    return DETREND;
}

bool Plot::getNormalizeFlag()
{
    return NORMALIZE;
}
...
void Plot::setTitle(std::string newTitle)
{
    title = newTitle;
    fillText();
}
void Plot::setAxisLables(std::string xLabel, std::string yLabel)
{
    xAxisLabel = xLabel;
    yAxisLabel = yLabel;
    fillText();
...
std::vector<TextLabel*> Plot::getText()
{
    return text;
}
...
void Plot::fillGridVertexArray()
{
    ...
    for (int i = rowVerts; i <= totalVerts; i += 12)
    {  // cols
        x += absOffsetX;
...
void Plot::fillText()
{
    ...
    text.push_back(new TextLabel(glm::vec3(left - 0.1, bottom, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string((int)refMinY)));
    text.push_back(new TextLabel(glm::vec3(left - 0.1,    top, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string((int)refMaxY)));
    text.push_back(new TextLabel(glm::vec3(right, bottom - 0.1, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string((int)refMaxX)));
    text.push_back(new TextLabel(glm::vec3( left, bottom - 0.1, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string((int)refMinX)));
    ...
    for (int i = 1; i < ROWS; i++)
    {
        ...
        text.push_back(new TextLabel(glm::vec3(left-0.1, y, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string(value)));
    }
    ...
    for (int i = 1; i < COLS; i++)
    {
        ...
        text.push_back(new TextLabel(glm::vec3(x, bottom - 0.1, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.1, std::to_string(value)));
    }
}
