...
glm::vec3 Button::getDrawLocation()
{
    return location;
}

TextLabel* Button::getLabelText()
{
    return  textList[0];
}
...
void Button::fillTextLabel()
{
    ...
    if (PRESSED)
    {
        textList.push_back(new TextLabel(location, glm::vec3 (1.0f, 0.7f, 0.27f), width * 0.7, textString));
    }
    else
    {
        textList.push_back(new TextLabel(location, color, width * 0.7, textString));
    }
}
